...
@Serializable
private data class GroupRow(val id: String, val name: String, val invite_code: String)

@Serializable
private data class GroupInsert(val name: String, val created_by: String)
...
class GroupRepository {
    ...
    suspend fun getMyGroups(): List<GroupSummary> {
        ...
        val memberships = client.postgrest["group_members"]
            .select(Columns.raw("group_id, groups(id, name, invite_code)")) {
        ...
        return memberships.mapNotNull { row ->
            ...
            GroupSummary(id = id, name = name, memberCount = count)
        }
    }
